...
def _settings(tmp_path) -> Settings:
    base = Settings(
        llm_base_url="", llm_api_key="", llm_model="", llm_max_tokens=512, port=8000, log_level="info",
...
class TestTwoStageRecall:
    ...
    async def test_tool_event_carries_hits_for_frontend(self, indexed):
        ...
        # 泛“露营灯”查询可由 SolarLamp 或 LumenGo 居首；本测试关注事件完整性，
        # 不把检索排序策略写死在前端事件契约上。
        assert hits and hits[0]["product_id"] in {"P1008", "P1039"}
