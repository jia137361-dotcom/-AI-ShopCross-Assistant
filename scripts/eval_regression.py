...
# 独立脚本不会经过应用 composition，需自行加载本地配置；否则 Judge 读取
# os.environ['LLM_BASE_URL'] 时会误报 KeyError，导致全部 case 变成 ERROR。
...
def render_report(results: list[dict]) -> str:
    lines = [
        f"# ShopCross 评测回归报告（{datetime.now().strftime('%Y-%m-%d %H:%M')}）",
        "",
