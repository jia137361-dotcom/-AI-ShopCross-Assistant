...
from app.presentation.account_auth import (
    AccountPrincipal,
    issue_token,
    login,
    register,
...
def build_app() -> FastAPI:
    ...
    async def lifespan(_: FastAPI):
        ...
        state["c"] = c
        state["connections"] = ConnectionManager(c.bus, c.conversation_store, c.settings.auth_secret)
        await c.startup()
    ...
    api = FastAPI(title="ShopCross 跨境电商 Agent", version="0.4.0", lifespan=lifespan)
    ...
    def require_principal(request: Request) -> AccountPrincipal:
        authorization = request.headers.get("authorization", "")
        token = authorization.removeprefix("Bearer ").strip() if authorization.startswith("Bearer ") else None
        return verify_token(token, container().settings.auth_secret)

    async def assert_session_owner(c: Container, session_id: str, principal: AccountPrincipal) -> None:
        session = await c.conversation_store.find_session(session_id)
        if session is not None and session["buyer_id"] != principal.buyer_id:
            # 以 404 响应，避免向未授权方确认该会话存在。
    ...
    async def register_account(body: dict) -> dict:
        principal = await register(container().db_engine, str(body.get("username", "")), str(body.get("password", "")))
        return {"token": issue_token(principal, container().settings.auth_secret), "username": principal.username}

    @api.post("/auth/login")
    async def login_account(body: dict) -> dict:
        principal = await login(container().db_engine, str(body.get("username", "")), str(body.get("password", "")))
        return {"token": issue_token(principal, container().settings.auth_secret), "username": principal.username}

    @api.get("/commerce/sessions/{session_id}/history")
    async def session_history(session_id: str, principal: AccountPrincipal = Depends(require_principal)) -> dict:
        """恢复前端对话、执行轨迹和商品卡所需的持久化数据。"""
        c = container()
        session = await c.conversation_store.find_session(session_id)
        if session is None:
            # 新建会话在首次发送消息前尚未落库；返回空历史是正常的初始化状态，
            # 不应让前端为一次预期的 GET 产生 404。
            return {"session_id": session_id, "turns": [], "events": []}
        await assert_session_owner(c, session_id, principal)
        turns = await c.conversation_store.list_turns(session_id)
        events = await c.conversation_store.list_events(session_id)
        return {
    ...
    async def list_sessions(limit: int = 30, principal: AccountPrincipal = Depends(require_principal)) -> dict:
        """返回当前买家的最近会话，供左侧会话栏切换。"""
        safe_limit = min(max(limit, 1), 100)
        return {"sessions": await container().conversation_store.list_sessions(principal.buyer_id, safe_limit)}

    @api.post("/commerce/intents", response_model=SubmitIntentResponse)
    async def submit_intent(body: SubmitIntentRequest, principal: AccountPrincipal = Depends(require_principal)) -> SubmitIntentResponse:
        c = container()
    ...
    async def submit_intent_async(body: SubmitIntentRequest, principal: AccountPrincipal = Depends(require_principal)) -> dict:
        c = container()
        session_id = body.shopping_session_id or f"session-{uuid.uuid4().hex[:8]}"
        await assert_session_owner(c, session_id, principal)
        intent = SubmitIntentInput(
    ...
    async def get_task(task_id: str, principal: AccountPrincipal = Depends(require_principal)) -> dict:
        c = container()
        if c.task_queue is None:
            raise HTTPException(status_code=503, detail="队列未启用")
        owner = await c.cache.get_raw(f"task-owner:{task_id}")
        if owner != principal.buyer_id:
    ...
    async def get_order(order_id: str, principal: AccountPrincipal = Depends(require_principal)) -> dict:
        try:
            return await container().query_order.execute_for_buyer(order_id, principal.buyer_id)
        except ValueError as err:
            raise HTTPException(status_code=404, detail=str(err)) from err

    @api.post("/commerce/orders/{order_id}/cancel")
    async def cancel_order_endpoint(order_id: str, body: CancelOrderRequest, principal: AccountPrincipal = Depends(require_principal)) -> dict:
        try:
            return await container().cancel_order.execute_for_buyer(order_id, body.reason, principal.buyer_id)
        except ValueError as err:
...
async def _queue_priority(c: Container, session_id: str) -> int:
    ...
    if not c.settings.queue_priority_enabled or not c.cache.enabled:
        return 0
    key = f"shopcross:turns:{session_id}"
    try:
...
async def _enqueue(c: Container, intent: SubmitIntentInput) -> str:
    ...
    await c.cache.set_json(f"task-owner:{task_id}", intent.buyer_id, _TASK_OWNER_TTL_SECONDS)
