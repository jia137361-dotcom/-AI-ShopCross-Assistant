...
def _prompt_fingerprint() -> str:
    ...
    读不到文件时返回固定值，不因此阻断启动。
    """
    path = Path(__file__).resolve().parent / "application" / "prompts" / "shopcross.yml"
    try:
...
async def build_container() -> Container:
    ...
    product_repo = InMemoryProductRepository()
    bus = TradeEventBus()
    langfuse = LangfuseObserver(settings)
    if langfuse.enabled:
        bus.attach_observer(langfuse)
        logger.info("Langfuse Trace 已启用：%s", settings.langfuse_base_url)
