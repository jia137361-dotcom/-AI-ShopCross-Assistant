...
class JsonFileConversationStore(ConversationStore):
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if record.get("kind") == "event":
                events.append(ConversationEventRecord(
                    session_id=session_id, type=record["type"],
                    payload=record.get("payload", {}), occurred_at=record.get("occurred_at", ""),
                ))
        return events[-limit:]
    ...
    async def list_sessions(self, buyer_id: str, limit: int = 30) -> list[dict]:
        ...
        for path in self._dir.glob("*.jsonl"):
            ...
            if session and session.get("buyer_id") == buyer_id:
                sessions.append({**session, "last_active_at": str(path.stat().st_mtime)})
        return sorted(sessions, key=lambda item: item["last_active_at"], reverse=True)[:limit]
