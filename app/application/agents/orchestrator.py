...
from app.infrastructure.langfuse_observer import LangfuseObserver
...
class MainAgentOrchestrator:
    def __init__(
        ...
        langfuse_observer: Optional[LangfuseObserver] = None,
    ) -> None:
    ...
    async def handle_intent(self, intent: SubmitIntentInput) -> SubmitIntentOutput:
        ...
        if self._langfuse_observer is not None:
            self._langfuse_observer.start_turn(session_id, intent.buyer_id, intent.raw_query)
        ...
        try:
            ...
            if self._langfuse_observer is not None:
                self._langfuse_observer.finish_turn(
                    session_id, final_text, int((time.monotonic() - started_at) * 1000),
                )
