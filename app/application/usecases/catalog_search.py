...
class CatalogSearchUseCase:
    ...
    async def execute(self, spec: ProductSearchSpec) -> dict:
        ...
        if self._embedder is not None and self._vector_index is not None:
            try:
                dense = await self._vector_recall(spec)
                bm25 = await self._bm25_recall(spec)
                scored = self._rrf_fuse(dense, bm25)
                recall_strategy = "hybrid_rrf"
            except Exception as err:  # noqa: BLE001 —— 召回基建异常必须降级而非失败
                logger.warning("向量召回不可用，降级关键词召回：%s", err)
                scored = []

        if recall_strategy == "hybrid_rrf" and scored:
            # 二阶段精排；失败降级按向量分排序
            try:
                scored = await self._rerank(spec, scored)
                recall_strategy = "hybrid_rrf_rerank"
                rerank_applied = True
    ...
    async def _bm25_recall(self, spec: ProductSearchSpec) -> list[tuple[float, Product]]:
        """线上词法召回：与离线 ESCI 的 BM25 + Dense + RRF 口径保持一致。"""
        ...
        return scores[:_RECALL_TOP_N]

    @staticmethod
    def _rrf_fuse(
        dense: list[tuple[float, Product]],
        bm25: list[tuple[float, Product]],
    ) -> list[tuple[float, Product]]:
        """按 Reciprocal Rank Fusion 融合两路候选，避免不同分数尺度不可比。"""
        scores: dict[str, float] = {}
        products: dict[str, Product] = {}
        dense_ranks: dict[str, int] = {}
        bm25_ranks: dict[str, int] = {}
        for source, ranking in (("dense", dense), ("bm25", bm25)):
            for rank, (_, product) in enumerate(ranking, start=1):
                products[product.product_id] = product
                scores[product.product_id] = scores.get(product.product_id, 0.0) + 1 / (_RRF_K + rank)
                (dense_ranks if source == "dense" else bm25_ranks)[product.product_id] = rank
        return sorted(
            ((score, products[product_id]) for product_id, score in scores.items()),
            # RRF 同分时优先词法排名：精确属性（如“抗造”）不应被向量并列的泛品类词吞掉。
            key=lambda pair: (
                -pair[0],
                bm25_ranks.get(pair[1].product_id, _RECALL_TOP_N + 1),
                dense_ranks.get(pair[1].product_id, _RECALL_TOP_N + 1),
                pair[1].product_id,
            ),
        )
